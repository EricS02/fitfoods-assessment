...
export class ProductCard extends Component {
  ...
  connectedCallback() {
    ...
    // Attach hover listeners for secondary image preview
    this.addEventListener("pointerenter", this.previewImage);
    this.addEventListener("pointerleave", this.resetImage);
  ...
  disconnectedCallback() {
    ...
    this.removeEventListener("pointerenter", this.previewImage);
    this.removeEventListener("pointerleave", this.resetImage);
  }

  #preloadNextPreviewImage() {
    const currentSlide =
      this.refs.slideshow?.slides?.[this.refs.slideshow?.current];
  ...
  #updateOverflowList() {
    ...
    const isActiveOverflowList = overflowList?.querySelector(
      '[slot="overflow"]',
    )
      ? true
      : false;
  ...
  previewImage(event) {
    ...
    const selectedColor = selectedOption.value?.toLowerCase();
    if (!selectedColor) return;

    const { slides = [] } = slideshow.refs;

    const secondarySlide = slides.find((slide) => {
      const img = slide.querySelector("img");
      const alt = img?.getAttribute("alt")?.toLowerCase() || "";

      // Match "- green -" pattern to avoid partial matches
      return (
        alt.includes(`- ${selectedColor} -`) &&
        !slide.hasAttribute("variant-image")
      );
    });

    if (secondarySlide) {
      const id = secondarySlide.getAttribute("slide-id");
      if (id) {
        slideshow.select({ id }, undefined, { animate: false });
      }
    }
  ...
  resetImage(event) {
    ...
    if (selectedOption) {
      const primaryId = selectedOption.dataset.optionMediaId;
      if (primaryId) {
        slideshow.select({ id: primaryId }, undefined, { animate: false });
        return;
      }
    }
